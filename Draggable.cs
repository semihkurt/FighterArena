...
public class Draggable : MonoBehaviour
{
        ...
        if (t != null)
        {
            ...
            if (candidateNode != null && thisEntity != null)
            {
                if (!candidateNode.IsOccupied)
                {
                    ...
                    Debug.Log("it is occupied");
                }
                Debug.Log("Candidate node or this entity is null");
            }
        }else{
    ...
    {
        RaycastHit2D hit = Physics2D.Raycast(cam.ScreenToWorldPoint(Input.mousePosition), Vector2.zero, Mathf.Infinity, releaseMask);
